...
RavObstacle::RavObstacle(std::string id, double attenuationPerWall, double attenuationPerMeter) :
...
void RavObstacle::setShape(Coords shape) {
	coords = shape;
...
const RavObstacle::Coords& RavObstacle::getShape() const {
	return coords;
}

const Coord RavObstacle::getBboxP1() const {
	return bboxP1;
}

const Coord RavObstacle::getBboxP2() const {
	return bboxP2;
}
...
namespace {
    ...
    bool isPointInRavObstacle(Coord point, const RavObstacle& o) {
		bool isInside = false;
        const RavObstacle::Coords& shape = o.getShape();
        RavObstacle::Coords::const_iterator i = shape.begin();
        RavObstacle::Coords::const_iterator j = (shape.rbegin()+1).base();
		for (; i != shape.end(); j = i++) {
...
double RavObstacle::calculateAttenuation(const Coord& senderPos, const Coord& receiverPos) const {
    ...
    const RavObstacle::Coords& shape = getShape();
    RavObstacle::Coords::const_iterator i = shape.begin();
    RavObstacle::Coords::const_iterator j = (shape.rbegin()+1).base();
	for (; i != shape.end(); j = i++) {
		Coord c1 = *i;
